...
class FilmStripWidget:
    ...
    def __init__(self, x: int, y: int, width: int, height: int):
        ...
        self.hovered_frame: Optional[Tuple[str, int]] = None
        ...
        for anim_name, frames in self.animated_sprite.frames.items():
            ...
            for frame_idx, frame in enumerate(frames):
                ...
                frame_rect = pygame.Rect(
                    frame_x, self.animation_label_height,
            ...
            if anim_name != list(self.animated_sprite.frames.keys())[-1]:
                sprocket_rect = pygame.Rect(
                    x_offset, 0, self.sprocket_width, self.frame_height + self.animation_label_height
                )
                self.sprocket_layouts.append(sprocket_rect)
                x_offset += self.sprocket_width
    
    def get_frame_at_position(self, pos: Tuple[int, int]) -> Optional[Tuple[str, int]]:
    ...
    def get_animation_at_position(self, pos: Tuple[int, int]) -> Optional[str]:
    ...
    def set_current_frame(self, animation: str, frame: int) -> None:
        ...
        if self.animated_sprite and animation in self.animated_sprite.frames:
            if 0 <= frame < len(self.animated_sprite.frames[animation]):
                self.current_animation = animation
                self.current_frame = frame
    
    def handle_click(self, pos: Tuple[int, int]) -> Optional[Tuple[str, int]]:
    ...
    def render_frame_thumbnail(self, frame, is_selected: bool = False, is_hovered: bool = False) -> pygame.Surface:
        ...
        if hasattr(frame, 'image') and frame.image:
            ...
            scaled_image = pygame.transform.scale(frame.image, (self.frame_width - 8, self.frame_height - 8))
        ...
        for hole_x in range(4, self.frame_width - 4, 8):
            ...
            # Bottom row holes
            pygame.draw.circle(frame_surface, self.sprocket_color, (hole_x, self.frame_height - 3), 1)
        
        # Add film strip edges
        pygame.draw.line(frame_surface, self.frame_border, (0, 0), (0, self.frame_height), 1)
        pygame.draw.line(frame_surface, self.frame_border, (self.frame_width - 1, 0), (self.frame_width - 1, self.frame_height), 1)
        ...
        if is_hovered:
            pygame.draw.rect(frame_surface, self.hover_color, (0, 0, self.frame_width, self.frame_height), 2)
    ...
    def render_sprocket_separator(self, x: int, y: int, height: int) -> pygame.Surface:
        ...
        pygame.draw.line(separator, self.frame_border, (self.sprocket_width - 1, 0), (self.sprocket_width - 1, height), 1)
    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        for anim_name, anim_rect in self.animation_layouts.items():
            ...
            text = font.render(anim_name, True, (255, 255, 255))
            ...
            if self.hovered_animation == anim_name:
                pygame.draw.rect(label_surface, self.hover_color, (0, 0, anim_rect.width, anim_rect.height), 2)
            
            surface.blit(label_surface, anim_rect)
        
        # Render frames
        for (anim_name, frame_idx), frame_rect in self.frame_layouts.items():
            if anim_name in self.animated_sprite.frames and frame_idx < len(self.animated_sprite.frames[anim_name]):
