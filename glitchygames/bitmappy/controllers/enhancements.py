...
class BitmappyMultiControllerEnhancements:
    ...
    def enhanced_navigation(self, controller_id: int, direction: str, amount: int = 1) -> bool:
        ...
        if direction == 'frame':
            ...
            if animations:
                current_index = (
                    animations.index(current_animation) if current_animation in animations else 0
                )
    ...
    def _record_controller_action(
        ...
        action_record = {'timestamp': time.time(), 'action': action, 'data': data}
    ...
    def get_controller_performance_stats(self, controller_id: int) -> dict[str, Any]:
        ...
        if self.controller_history.get(controller_id):
    ...
    def optimize_performance(self) -> dict[str, Any]:
        ...
        if (
            optimization_results['controllers_cleaned'] > 0
            or optimization_results['cache_cleared'] > 0
        ):
