#!/usr/bin/env python3
"""GlitchyGames UI widget classes.

This module contains all UI widget implementations including text sprites,
buttons, menus, sliders, color wells, dialogs, and other interactive components.
"""
...
class MenuItem(BitmappySprite):
    ...
    @override
    def add(self: Self, *groups: pygame.sprite.LayeredDirty[Any]) -> None:  # type: ignore[override]
...
class InputBox(Sprite):
    ...
    def on_mouse_up_event(self: Self, event: HashableEvent) -> None:
...
class SliderSprite(BitmappySprite):
    ...
    def _init_text_sprite(
        ...
        self.text_sprite.on_left_mouse_button_down_event = handle_text_click  # type: ignore[assignment]
...
class InputDialog(BitmappySprite):
    ...
    def on_input_box_cancel_event(self: Self, control: object) -> None:
        ...
        self.on_cancel_event(event=control, trigger=control)  # type: ignore[arg-type]
...
class Scrollbar:
    ...
    def handle_mouse_up(self, mouse_pos: tuple[int, int]) -> bool:
    ...
    def _handle_regular_key(self, event: HashableEvent) -> None:
        ...
        if event.key == pygame.K_RETURN:
            ...
            if self.cursor_pos > 0:
                new_cursor_pos = self.cursor_pos - 1
                self.text = (
                    self._original_text[: self.cursor_pos - 1]
                    + self._original_text[self.cursor_pos :]
                )
                self.cursor_pos = new_cursor_pos
