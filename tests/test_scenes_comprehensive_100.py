...
This module tests all missing functionality with proper refactoring for testability.
"""
...
class TestScenesComprehensive100(unittest.TestCase):
    """Test coverage for comprehensive Scenes functionality to reach 100%."""
    ...
    def test_scene_manager_play_method_fps_event_lines_213_218(self):
        """Test SceneManager play method FPS event (covers lines 213-218)."""
        scene_manager = SceneManager()
        ...
        with patch("time.perf_counter") as mock_time, \
            ...
            # Mock time progression to trigger FPS event
            mock_time.side_effect = [0.0, 0.016, 0.032, 0.048, 0.064]  # Simulate 60 FPS
            mock_clock_instance = Mock()
            mock_clock_instance.get_fps.return_value = 60.0
            mock_clock_class.return_value = mock_clock_instance
            mock_game_engine.process_events = Mock()
        ...
        mock_event_post.assert_called()

    def test_scene_manager_play_method_scene_switch_line_220(self):
        """Test SceneManager play method scene switch (covers line 220)."""
        scene_manager = SceneManager()
        
        # Create a mock scene with next scene
        mock_scene = Mock()
        mock_scene.target_fps = 60
        mock_scene.rects = []
        mock_scene.dirty = 1
        ...
        mock_next_scene.target_fps = 30
        mock_next_scene.NAME = "NextScene"
        mock_next_scene.load_resources = Mock()
        mock_next_scene.setup = Mock()
        mock_next_scene.background = self.mock_surface
        mock_next_scene.rects = []
        mock_next_scene.dirty = 1
        mock_next_scene.dt_tick = Mock()
        mock_next_scene.update = Mock()
        mock_next_scene.render = Mock()
        mock_next_scene.next_scene = None  # No further next scene
        
        mock_scene.next_scene = mock_next_scene
        ...
        # Verify scene switch occurred
        self.assertEqual(scene_manager.active_scene, mock_next_scene)

    def test_scene_manager_play_method_quit_logging_lines_224_228(self):
        """Test SceneManager play method quit logging (covers lines 224-228)."""
        scene_manager = SceneManager()
        ...
        # Verify quit logging was called
        mock_log.info.assert_called()
