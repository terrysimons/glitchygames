...
class TestWallCollisions:
    ...
    def test_ball_bounces_off_top_wall(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(3.0, -2.0)  # Moving up and right
        ...
        assert ball.speed.x == 3.0  # Unchanged
        assert ball.speed.y == 2.0   # Reversed from -2.0 to +2.0
    ...
    def test_ball_bounces_off_bottom_wall(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(3.0, 2.0)  # Moving down and right
    ...
    def test_wall_collision_preserves_speed_magnitude(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(3.0, 4.0)  # Magnitude = 5.0
        initial_magnitude = math.sqrt(3.0**2 + 4.0**2)
        ...
        assert abs(final_magnitude - initial_magnitude) < 0.01  # Within tolerance
    ...
    def test_wall_collision_does_not_change_horizontal_speed(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(5.0, -3.0)  # Moving right and up
        ball.rect.y = 0  # At top wall (triggers bounce condition)
        ...
        assert ball.speed.x == 5.0  # Unchanged
        assert ball.speed.y == 3.0  # Reversed from -3.0 to +3.0
...
class TestPaddleCollisions:
    ...
    def test_ball_bounces_off_left_paddle(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(-3.0, 2.0)  # Moving left and down
        initial_magnitude = math.sqrt(3.0**2 + 2.0**2)
        ...
        assert ball.speed.x > 3.0  # Reversed and increased
        assert ball.speed.y > 2.0  # Increased
        final_magnitude = math.sqrt(ball.speed.x**2 + ball.speed.y**2)
    ...
    def test_ball_bounces_off_right_paddle(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(3.0, 2.0)  # Moving right and down
        initial_magnitude = math.sqrt(3.0**2 + 2.0**2)
        ...
        # Expected: Direction reversed, speed increased (using logarithmic scaling)
        # The speed_up method uses logarithmic scaling, not simple multiplication
        assert ball.speed.x < -3.0  # Reversed and increased (negative)
        assert ball.speed.y > 2.0   # Increased
        final_magnitude = math.sqrt(ball.speed.x**2 + ball.speed.y**2)
        assert final_magnitude > initial_magnitude  # Speed increased
    ...
    def test_paddle_collision_reverses_horizontal_direction(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(-4.0, 1.0)  # Moving left
        
        # Action: Simulate paddle collision
        ball.speed.x *= -1  # Reverse direction
        ball.speed_up(1.15)  # Speed increase
...
class TestBallToBallCollisions:
    ...
    def test_ball_to_ball_collision_conserves_energy(self, mock_pygame_patches):
        ...
        ball1 = BallSprite()
        ball2 = BallSprite()
        ball1.speed = Speed(2.0, 0.0)  # Moving right
        ball2.speed = Speed(0.0, 3.0)  # Moving down
        
        # Calculate initial total kinetic energy
        initial_energy = 0.5 * (2.0**2 + 0.0**2 + 0.0**2 + 3.0**2)  # 6.5
        ...
        assert abs(final_energy - initial_energy) < 0.01  # Within tolerance
    ...
    def test_ball_to_ball_collision_exchanges_momentum(self, mock_pygame_patches):
        ...
        ball1 = BallSprite()
        ball2 = BallSprite()
        ball1.speed = Speed(3.0, 0.0)  # Moving right
        ball2.speed = Speed(0.0, 0.0)  # Stationary
        ...
        assert ball1.speed.x == 0.0  # Now stationary
        assert ball1.speed.y == 0.0
        assert ball2.speed.x == 3.0  # Now moving
        assert ball2.speed.y == 0.0
    ...
    def test_ball_to_ball_collision_cooldown(self, mock_pygame_patches):
        ...
        assert ball1.collision_cooldowns[ball2_id] > current_time - 2.0
        assert ball2.collision_cooldowns[ball1_id] > current_time - 2.0
    ...
    def test_ball_to_ball_collision_separation(self, mock_pygame_patches):
        ...
        overlap = 5.0
        separation_distance = max(overlap, 2.0)
        ball1.rect.x -= separation_distance * 0.5
        ball2.rect.x += separation_distance * 0.5
        
        # Expected: Balls are separated
        distance = abs(ball2.rect.x - ball1.rect.x)
        assert distance >= 2.0  # Minimum separation
    ...
    def test_ball_to_ball_collision_elastic_physics(self, mock_pygame_patches):
        ...
        ball1 = BallSprite()
        ball2 = BallSprite()
        ball1.speed = Speed(4.0, 0.0)  # Moving right
        ball2.speed = Speed(-2.0, 0.0)  # Moving left
        
        # Calculate initial total momentum
        initial_momentum_x = ball1.speed.x + ball2.speed.x  # 2.0
        initial_momentum_y = ball1.speed.y + ball2.speed.y  # 0.0
        ...
        final_momentum_x = ball1.speed.x + ball2.speed.x  # 2.0
        final_momentum_y = ball1.speed.y + ball2.speed.y  # 0.0
        
        # Expected: Momentum conserved
        assert abs(final_momentum_x - initial_momentum_x) < 0.01
        assert abs(final_momentum_y - initial_momentum_y) < 0.01
...
class TestIntegrationCollisions:
    ...
    def test_mixed_collision_scenarios(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(3.0, -2.0)  # Moving right and up
        ball.rect.y = 0  # At top wall (triggers bounce condition)
        ...
        assert paddle_speed > 3.0  # Paddle collision increased speed
    ...
    def test_multiple_balls_collision_behavior(self, mock_pygame_patches):
        ...
        ball1.speed = Speed(2.0, -1.0)  # Add vertical movement for wall collision
        ball2.speed = Speed(2.0, 3.0)  # Add horizontal movement for paddle collision
        ball3.speed = Speed(-1.0, 1.0)
        ...
        assert ball1.speed.x == -1.0  # Ball-to-ball collision worked
        assert ball3.speed.x == 2.0
    ...
    def test_collision_edge_cases(self, mock_pygame_patches):
        ...
        ball = BallSprite()
        ball.speed = Speed(10.0, -1.0)  # Very fast horizontal with vertical movement
