...
class TestFilmStripNavigation:
    ...
    def setup_method(self):
        ...
        def create_mock_frame():
            frame = Mock()
            frame.image = Mock()
            frame.image.get_size.return_value = (32, 32)  # Return a tuple for get_size()
        ...
        self.mock_sprite._animations = {
            ...
            "attack": [create_mock_frame(), create_mock_frame(), create_mock_frame(), create_mock_frame()]  # 4 frames
        }
        
        # Create scene with required options
        self.scene = BitmapEditorScene(options={})
        self.scene.max_visible_strips = 2
        self.scene.film_strip_scroll_offset = 0
        ...
        self.scene.canvas.current_frame = 0
    ...
    def test_up_arrow_navigation(self):
        ...
        assert self.scene.canvas.current_frame == 0
    ...
    def test_left_arrow_frame_navigation(self):
        ...
        self.scene.canvas.current_animation = "idle"
        self.scene.canvas.current_frame = 2
    ...
    def test_scroll_boundaries(self):
        ...
        self.scene.film_strip_scroll_offset = 0
        self.scene.scroll_film_strips_up()
        assert self.scene.film_strip_scroll_offset == 0
        
        # Try to scroll down from max offset (should not exceed max)
        self.scene.film_strip_scroll_offset = 2  # Max for 4 animations with 2 visible
        self.scene.scroll_film_strips_down()
        assert self.scene.film_strip_scroll_offset == 2
    ...
    def test_auto_scroll_when_animation_already_visible(self):
        ...
        # Set up with 4 animations, only 2 visible
        self.scene.max_visible_strips = 2
        self.scene.film_strip_scroll_offset = 1  # Showing animations 1 and 2
    ...
    def test_film_strip_selection_state(self):
        ...
        if "walk" in self.scene.film_strips:
            ...
            assert walk_strip.is_selected == True
    ...
    def test_switch_to_film_strip(self):
        ...
        self.scene._switch_to_film_strip("jump", 0)
        
        # Check global selection state
        assert self.scene.selected_animation == "jump"
        assert self.scene.selected_frame == 0
    ...
    def test_dirty_marking_on_selection_change(self):
        ...
        self.scene._switch_to_film_strip("attack", 0)
    ...
    def test_keyboard_navigation_with_auto_scroll(self):
        ...
        assert self.scene.canvas.current_animation == "attack"
        assert self.scene.film_strip_scroll_offset == 2  # Should show animations 2 and 3
