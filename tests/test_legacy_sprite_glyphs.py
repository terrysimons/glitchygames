...
class TestLegacySpriteGlyphs(unittest.TestCase):
    ...
    def test_legacy_sprite_basic_save(self):
        ...
        self.assertEqual(config.get("sprite", "name"), "test_legacy")
        ...
        for char in color_sections:
            self.assertTrue(config.has_option(char, "red"))
            self.assertTrue(config.has_option(char, "green"))
            self.assertTrue(config.has_option(char, "blue"))
            ...
            self.assertLessEqual(blue, 255)

    def test_legacy_sprite_character_mapping(self):
        ...
        self.assertIn("#", color_sections)  # Black should map to '#'
        self.assertIn("@", color_sections)  # Red should map to '@'
        
        # Check that other colors use universal characters
        other_colors = [s for s in color_sections if s not in ['#', '@']]
        self.assertEqual(len(other_colors), 2, "Should have 2 other color sections")
        
        for char in other_colors:
            self.assertIn(char, glyphs, f"Character '{char}' should be in universal glyphs")
    ...
    def test_legacy_sprite_single_color(self):
        ...
        self.assertEqual(len(color_sections), 1, "Should have only one color section")
    ...
    def test_legacy_sprite_pixel_data_consistency(self):
        ...
        rows = pixels.split('\n')
        self.assertEqual(len(rows), 3, "Should have 3 rows")
        
        for row in rows:
            self.assertEqual(len(row), 3, "Each row should have 3 characters")
    ...
    def test_legacy_sprite_empty_surface(self):
        ...
        # Should have sprite section
        self.assertTrue(config.has_section("sprite"))
        self.assertEqual(config.get("sprite", "name"), "test_empty")
        
        # Should have no color sections
        glyphs = SPRITE_GLYPHS.strip()
        sections = config.sections()
        color_sections = [s for s in sections if len(s) == 1 and s in glyphs]
        self.assertEqual(len(color_sections), 0, "Should have no color sections")

    def test_legacy_sprite_grayscale(self):
    ...
    def test_legacy_sprite_character_order(self):
        ...
        for char in expected_chars:
            self.assertIn(char, color_sections, f"Character '{char}' should be used")
